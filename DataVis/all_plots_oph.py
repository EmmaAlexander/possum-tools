...
def main(args):
	...
	for i in range(0,nsrc):
		...
		if 1==1:
			...
			#add EMU data
			if EMUSB!='NaN':
				red_data,red_header=fitsopen(imagefolder+src+'_widePOSSUM.fits')
			...
			ax1.set_xlabel('Right Ascension [J2000]')
			ax1.set_ylabel('Declination [J2000]')
			ellipse= Ellipse(xy=(xmin+bmaj_pix,ymin+bmaj_pix),width=bmaj_pix,height=bmin_pix,angle=bpa+90,edgecolor='gray',fc='gray',lw=0.5)
			ax1.add_patch(ellipse)
			cbar=plt.colorbar(c,fraction=0.0475, pad=0.005)
